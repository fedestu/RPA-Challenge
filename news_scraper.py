...
class NewsScraper:
    ...
    #         try:                
    #             # Search and select the specific category in the "Type" section.
    #             categories = self.browser.find_elements("css:.search-filter-input.SearchFilterInput label.checkbox-input-label")
    #             category_found = False
    #             for category in categories:
    #                 if category_name in self.browser.get_text(category):
    #                     checkbox = self.browser.find_element("css:input[type='checkbox']", parent=category)
    ...
    # Método a quitar porque se estaría reemplazando con la "selección" en la url
    ...
    #             self.browser.wait_until_element_is_visible(sort_select_locator, timeout=10)

    #             current_value = self.browser.get_selected_list_value(sort_select_locator)

    #             if current_value != "1":  
    #                 self.browser.select_from_list_by_value(sort_select_locator, "1")
    #                 logging.info("Sort option set to 'Newest'.")
    #                 time.sleep(2)
    #             else:
    #                 logging.info("The 'Newest' sorting option is already selected.")

    #             self.browser.wait_until_element_is_visible("css:.search-results-module-results-header", timeout=30)
    #             logging.info("The 'Newest' sort option has been applied and the page has been refreshed.")
    #             return

    #         except Exception as e:
    #             logging.error(f"Attempt {attempts + 1} to sort by most recent failed: {str(e)}")
    #             self.browser.reload_page()
    #             attempts += 1

    #     logging.error("Failed to sort by Newest after multiple attempts.")
    ...
    #             try:
    #                 timestamp_elements = self.browser.find_elements("css:p.promo-timestamp", parent=article)
    #                 if not timestamp_elements:
    #                     logging.warning("Timestamp not found for article, skipping...")
    #                     continue

    #                 date_str = self.browser.get_element_attribute(timestamp_elements[0], "data-timestamp")
